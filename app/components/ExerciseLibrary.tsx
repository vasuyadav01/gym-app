  ...
  return (
    <div className="flex flex-col h-full bg-[var(--app-bg)]">
      ...
      <div className="flex items-center justify-between px-4 pt-5 pb-3">
        <h2 className="text-lg font-bold text-[var(--app-text)]">Add Exercise</h2>
        {onClose && (
          <button
            onClick={onClose}
      ...
      <div className="px-4 pb-3">
        <div className="flex items-center gap-2 bg-[var(--app-card)] rounded-xl px-3 py-2.5 border border-[var(--app-border)]">
      ...
      <div className="flex-1 overflow-y-auto">
        {visibleCategories.length === 0 ? (
          ...
          visibleCategories.map((category) => (
            <div key={category}>
              {/* Section header */}
              <div className="flex items-center gap-2 px-4 py-2 bg-[var(--app-card)] sticky top-0 z-10 border-b border-[var(--app-border)]">
              ...
              {grouped[category]!.map((exercise) => {
                ...
                return (
                  <button
                    ...
                    disabled={isAdded}
                    className={`w-full flex items-center justify-between px-4 py-3.5 border-b border-[var(--app-border)] text-left transition-colors ${
                      isAdded ? "opacity-40" : "hover:bg-[var(--app-card2)]"
                    }`}
                  >
                    <div className="min-w-0 flex-1">
                      <p className="font-semibold text-[var(--app-text)] text-sm">{exercise.name}</p>
                      <p className="text-xs text-[var(--app-text-muted)] mt-0.5 capitalize">
